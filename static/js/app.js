...
(() => {
  ...
  /* ---------------- Drag & drop video uploader (shared by merge + clip tabs) --- */
  let mergeDropzone = null;
  let clipDropzone = null;
  function initDropzone() {
    mergeDropzone = initDropzoneGeneric('video-dropzone', 'videos', 'video-queue', 'video-count-pill');
  }
  function initClipDropzone() {
    clipDropzone = initDropzoneGeneric('clip-video-dropzone', 'clip_videos', 'clip-video-queue', 'clip-video-count-pill');
  }

  function initDropzoneGeneric(zoneId, inputId, queueId, pillId) {
    const zone = $(zoneId);
    const input = $(inputId);
    const queue = $(queueId);
    const pill = $(pillId);
    if (!zone || !input) return { getFiles: () => [], reset: () => {} };
    let files = [];
    ...
    function renderQueue() {
      ...
      if (pill) {
        pill.style.display = files.length ? 'inline-flex' : 'none';
        pill.textContent = `${files.length} file${files.length === 1 ? '' : 's'}`;
      }
    }
  ...
  function initMergeForm() {
    ...
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const files = mergeDropzone.getFiles();
      if (files.length === 0) {
  ...
  function initClipForm() {
    ...
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const files = clipDropzone.getFiles();
      if (files.length === 0) {
        toast('Please upload a video to clip.', 'error');
        return;
      }
      const fd = new FormData(form);
      fd.delete('videos');
      ...
      const progress = simulateProgress('clip', ['Uploading video…', 'Slicing clips…', 'Applying safety filters…', 'Finalising…']);
      try {
